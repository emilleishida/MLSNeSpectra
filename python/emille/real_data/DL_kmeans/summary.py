...
op2 = open(path1, 'w')
op2.write('comp \t groups \t silhouette \t dunn \t davis \t vrc \n')
...
for pcs in range(2, 10):
    for groups in range(2, 10):
        op1 = open('info/quality_' + str(pcs) + 'comp_' + str(groups) + 'groups.info', 'r')
        lin1 = op1.readlines()
        op1.close()
        ...
        silhouette[str(pcs) + 'comp_' + str(groups) + 'g'] = float(data1[-4][-1])
        dunn[


str(pcs) + 'PC_' + str(groups) + 'g'] = float(data1[-3][-1])
        davis[str(pcs) + 'comp_' + str(groups) + 'g'] = float(data1[-2][-1])
        vrc[str(pcs) + 'comp_' + str(groups) + 'g'] = float(data1[-1][-1])
