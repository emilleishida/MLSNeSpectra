...
branch_code = []
for kk in [0,1,2,3]:
    temp_code = np.array(color_branch[0]) == color_branch[2]['color'][kk]
...
legs=fig1.legend(panels, ['Group 1', 'Group 2'], loc = (0.0, 0.889), title='Kmeans classfication', fontsize=22)
...
for j in [0,1,2,3]:
    panels2[j] = plt.scatter(Y[branch_code[j],0], Y[branch_code[j],1], marker=mark[j], color=c[j], s=size[j])
    if color_branch[2]['name'][j] != 'nan':
        names.append(color_branch[2]['name'][j])
    else:
        names.append('peculiar')
legs2=fig2.legend(panels2, names, loc = (0.0, 0.819), title='Branch classification', fontsize=22)
...
plt.tight_layout()
plt.savefig("kmeans_branch_isomap_2g.pdf", format='pdf',dpi=1000)
